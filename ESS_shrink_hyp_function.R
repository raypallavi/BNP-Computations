...
mon.inc.shrink.hyp=function(y,x,N,eta,mcmc,brn,thin,nu.in,l.in,tau.in,sig.in,xi0.in,
                            xi.in,lam.in,xi0.fix,xi.fix,lam.fix,tau.fix,sig.fix,
  ...
  # nu.in, l.in, tau.in, sig.in, xi0.in, xi.in, lam.in : initial values (supplied by user)
  ...
  # OUTPUT: Posterior samples on nu,l,xi,xi0,tau,sig and fhat with posterior mean, 95% CI of fhat
  
  if(!missing(lam.fix) && length(unique(lam.fix))==1 && unique(lam.fix)==1)
    return(mon.inc.ESS.hyp(y,x,N,eta,mcmc,brn,thin,nu.in,l.in,tau.in,sig.in,xi0.in,xi.in,
  ...
  for(i in 1:em){
    ...
    y_tilde = y - xi0
    if(missing(xi.fix)){
      nu.xi = as.vector(samp.WC(my_knots,nu_out,l_out,tau))
    ...
    if(missing(lam.fix)){
      ...
      lam_out = ESS(lam_in,nu.lam,y_tilde,t(t(X)*xi_in),sig,eta)
    ...
    # sampling gamma_j's:
    if(missing(lam.fix)){
      gam_out = rexp(l1,rate = 0.5*(1+lam_in^2))
      gam_in=gam_out
    }
  ...
  return(list("time"=tm,"rej.prop"=round(cnt/em,3),"nu_sam"=nu_sam,"ell_sam"=ell_sam,
              "xi_sam"=xi_sam,"lam_sam"=lam_sam,"xi0_sam"=xi0_sam,"sig_sam"=sig_sam,
...
# con.ESS.hyp=function(y,x,N,eta,mcmc,brn,thin,nu.in,l.in,tau.in,sig.in,xi0.in,xi1.in,xi.in,sseed,
...
#   for(i in 1:em){
#     # sampling from \nu and \ell
#     MH.out = nu.MH2(nu_in,l_in,tau,xi_in,my_knots,seed = i)
#     nu_out = MH.out$nu
#     l_out = MH.out$l
#     cnt = cnt + MH.out$count
#     #K = covmat(my_knots,nu_out,l_out)
#     #K_inv = tinv(K)
#     L_inv = MH.out$L_inv
#     
#     # sampling \Xi:
#     y_tilde = y - xi0 - xi1*x
#     nu.ess = as.vector(samp.WC(my_knots,nu_out,l_out,tau,samp.size = 1,sseed = i))
#     xi_out = ESS(xi_in,nu.ess,y_tilde,X,sig,eta,seeds=i)
#     xi_out = sapply(xi_out,function(z) return(max(0,z)))
#     
#     set.seed(2*i)
#     
...
#   tm=proc.time()-ptm
#   return(list("time"=tm,"rej.prop"=round(cnt/em,3),"nu_sam"=nu_sam,"ell_sam"=ell_sam,
#               "xi_sam"=xi_sam,"xi0_sam"=xi0_sam,"xi1_sam"=xi1_sam,"sig_sam"=sig_sam,
#               "tau_sam"=tau_sam,"fhat_sam"=fhat_sam,"fmean"=fmean,"f_low"=f_low,"f_upp"=f_upp))
# }
